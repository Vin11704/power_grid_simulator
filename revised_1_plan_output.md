...
The repo is a bare `uv init --package` scaffold: the only Python file is [**init**](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/src/power_grid_simulator/__init__.py)[.py](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/src/power_grid_simulator/__init__.py) with a `main()` print stub. There is no simulator, no Streamlit app, and no tests.

[AI\_prompt.md](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/AI_prompt.md) specifies a live Streamlit power grid simulator for an Arctic research station: one diesel generator (G101), one circuit breaker (CB101), and a frequency sensor (FS101) on a bus that must be held near 50.00 Hz. The operator flips G101 and CB101 while the simulation runs in 1-second ticks, and a safety interlock overrides the operator to keep bus frequency inside 49.50–50.50 Hz. The outcome is a working app plus a unit-tested, UI-independent control-logic module.
...
2. **No markdown is written.** `AI_prompt.md` says to leave all markdown untouched, so README.md (currently truncated mid-sentence at `- cont`) and the `AI_USAGE.md` deliverable named in [power\_grid\_simulator.txt](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/power_grid_simulator.txt) are **out of scope** — the user writes those. Flagged here so it is not mistaken for an oversight.
3. **Layout** keeps the existing `uv_build` src-layout: modules in the package, `tests/` at root. **No dependency management** — the user installs `altair`, `pandas` and `pytest` manually, and [pyproject.toml](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/pyproject.toml) is not to be edited at all.
...
| File | Action |
| --- | --- |
| [simulator.py](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/src/power_grid_simulator/simulator.py) | new — `PowerGridSimulator`, no Streamlit/pandas import |
| [app.py](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/src/power_grid_simulator/app.py)             | new — Streamlit UI, Altair charts                      |
| [test\_simulator.py](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/tests/test_simulator.py)         | new — control-logic unit + property tests              |

Three new files, no edits to anything existing. Leave [pyproject.toml](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/pyproject.toml) and the `main()` stub untouched, and no markdown. Per [CLAUDE.md](vscode-webview://003b50mldg4ffbo5uahf0ae8a603n07m08isk7vbhqs8h48f1ef2/CLAUDE.md), comment out rather than delete if anything ever must change.
...
1. **Bus frequency** — a mark_line trace layered with two dotted horizontal reference lines marking the safe limits, each with a text label:
    ```
    limits = pd.DataFrame({
        "limit": [UNDER_FREQ, OVER_FREQ],                      # 49.50, 50.50
        "label": ["Under-frequency limit 49.50 Hz", "Over-frequency limit 50.50 Hz"],
    })

    line = alt.Chart(hist).mark_line(point=True).encode(
        x=alt.X("t:Q", title="Time (s)"),
        y=alt.Y("frequency:Q", title="Bus frequency (Hz)",
    ...
    rules = alt.Chart(limits).mark_rule(
        strokeDash=[2, 3], color="red", size=1.5,               # dotted
    ).encode(y="limit:Q")
    labels = alt.Chart(limits).mark_text(
        align="left", baseline="bottom", dx=4, dy=-3, fontSize=11, color="red",
    ).encode(x=alt.value(5), y=alt.Y("limit:Q"), text="label:N")

    freq_chart = alt.layer(rules, labels, line).properties(
        height=280, title="Bus frequency (FS101)")
    ```
    The 0.10 Hz of headroom on each side keeps both dotted lines clear of the frame edge — the band is 1.00 Hz wide and steady-state operation spans ~0.96 Hz of it, so autoscaling would put the rules right on the axis boundary and clip their labels. With the headroom, both labels sit just above their line inside the plot area.
...
Control logic only — no Streamlit, pandas or altair import anywhere in the tests.
...
Handled manually by the user; no `pyproject.toml` edits. `app.py` imports `streamlit`, `altair` and `pandas` (the latter two are currently present transitively via Streamlit — 6.2.2 and 3.0.5, which the verified chart APIs were tested against). The tests import only the standard library plus `pytest`.
...
```powershell
uv run pytest tests/ -v              # control logic green FIRST (CLAUDE.md: TDD)
uv run streamlit run src/power_grid_simulator/app.py

```

No `conftest.py` and no pytest config needed — the project is already editable-installed (`power_grid_simulator.pth` in the venv), so `from power_grid_simulator.simulator import ...` resolves from `tests/` as-is.
